...
This skill ships bundled inside the `@try-works/dsh-anti-slop` DeepSeek Harness bundle. In DSH, the skill body is registered by the bundle plugin with a directory `resourceBase` pointing at this skill directory, so the instructions above can be followed verbatim (the bundled assets live under `assets/anti-slop/` in the skill directory). The bundle also exposes:

- the `anti_slop_lint` tool — run the vendored anti-slop Oxlint rules against a target without touching the target's config (see the tool schema for `target`, `rules`, `effect`, `fix`, `severity`);
- the `/anti-slop` command — report the target project's anti-slop install status and how to use the tool;
- a standing system-prompt section describing when to use `anti_slop_lint` vs this skill (the skill handles a persistent local install; the tool handles one-off linting without configuration changes).

The vendored rule sources are byte-identical to upstream revision `6d538555cb151d4121ed51a27db81890eacf8ae9` (https://github.com/dmmulroy/anti-slop), maintained by `scripts/sync-vendored.mjs` with a recorded hash manifest at `scripts/vendored-manifest.json`.
